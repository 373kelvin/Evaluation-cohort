"""Recover the data model: parse SQL DDL, Django ORM, else infer from queries."""
import re
from .model import Table, Column, Relationship
from .scan import iter_files, read
from . import django_scan
from . import orm_scan
from . import readme_scan
...
def analyze_schema(project, root):
    ...
    if ddl_tables:
        ...
        django_scan.detect_django_models(project, root)
        if project.tables:
            return
        if orm_scan.detect_sqlalchemy(project, root):
            return
        if orm_scan.detect_prisma(project, root):
            return
        if react_scan.detect_json_server_schema(project, root):
            return
        if readme_scan.detect_readme_entities(project, root):
            return
